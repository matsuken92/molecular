...
def hold_out_lgb_validation(X_train, X_valid, y_train, y_valid, params, columns=None,
...
tda_radius_df_02 = pd.read_csv(save_path/"tda_radius_df_v002.csv", index_col=0)
train = train.merge(tda_radius_df_02, on="molecule_name", how="left")
test = test.merge(tda_radius_df_02, on="molecule_name", how="left")
print("finish additional features process.")
...
# y = [0]*len(X) + [1]*len(X_test)
# X = pd.concat([X, X_test], axis=0)
molname_train = train.molecule_name.unique()
molname_test  = test.molecule_name.unique()

molname_train_train, molname_train_valid = train_test_split(molname_train, test_size=0.2, random_state=42)
molname_test_train, molname_test_valid = train_test_split(molname_test, test_size=0.2, random_state=42)
molname_train_ = np.hstack([molname_train_train, molname_test_train])
molname_valid  = np.hstack([molname_train_valid, molname_test_valid])

train_train = train[train.molecule_name.isin(molname_train_)]
train_valid = train[~train.molecule_name.isin(molname_train_)]

test_train = test[test.molecule_name.isin(molname_valid)]
test_valid = test[~test.molecule_name.isin(molname_valid)]

X = pd.concat([train_train, test_train], axis=0)[use_cols.good_columns]
X_valid = pd.concat([train_valid, test_valid], axis=0)[use_cols.good_columns]
y_train = np.array([0]*len(train_train) + [1]*len(test_train))
y_valid = np.array([0]*len(train_valid) + [1]*len(test_valid))
...
result_dict_lgb = hold_out_lgb_validation(X_train=X, X_valid=X_valid,
