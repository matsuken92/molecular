...
def add_shelding_tensor_feat(df):
    shelding_tensor = pd.read_csv("../input/magnetic_shielding_tensors.csv")
    shelding_tensor_diag = shelding_tensor[["XX", "YY", "ZZ"]]
    shelding_tensor["diag_range"] = shelding_tensor_diag.max(axis=1) - shelding_tensor_diag.min(axis=1)

    for atom_idx in [0, 1]:
        df = pd.merge(df, shelding_tensor[["molecule_name", "atom_index", "diag_range"]], how='left',
                      left_on=['molecule_name', f'atom_index_{atom_idx}'],
                      right_on=['molecule_name', 'atom_index'])

        df = df.drop('atom_index', axis=1)
        df = df.rename(columns={'diag_range': f'diag_range{atom_idx}'})
    return df[["id", "diag_range0", "diag_range1"]]
####################################################################################################
# Settings
...
train_ = pd.read_csv("../input/train.csv")
test_ = pd.read_csv("../input/train.csv")
shelding_tensor_feat_train = add_shelding_tensor_feat(train_)
shelding_tensor_feat_test = add_shelding_tensor_feat(test_)
del train_
del test_
gc.collect()
train = train.merge(shelding_tensor_feat_train, on="id", how="left")
test = test.merge(shelding_tensor_feat_test, on="id", how="left")
