...
def add_shelding_vec_feat(df):
    shelding_tensor = pd.read_csv("../input/magnetic_shielding_tensors.csv")
    structures = pd.read_csv("../input/structures.csv")
    shield_structure = shelding_tensor.merge(structures, on=["molecule_name", "atom_index"], how="left")
    shield_vec = shield_structure.apply(lambda row: np.dot([[row.XX, row.XY, row.XZ],
                                                            [row.YX,row.YY,row.YZ],
                                                            [row.ZX,row.ZY,row.ZZ]],
                                                            [row.x, row.y, row.z, ]),axis=1, result_type="expand")
    shield_vec.columns = ["shield_vec_x", "shield_vec_y", "shield_vec_z"]
    shelding_tensor = pd.concat([shelding_tensor[["molecule_name", "atom_index"]], shield_vec],axis=1)
    for atom_idx in [0, 1]:
        df = pd.merge(df, shelding_tensor[["molecule_name", "atom_index", "shield_vec_x", "shield_vec_y", "shield_vec_z"]],
                      how='left',
                      left_on=['molecule_name', f'atom_index_{atom_idx}'],
                      right_on=['molecule_name', 'atom_index'])

        df = df.drop('atom_index', axis=1)
        df = df.rename(columns={f"shield_vec_{i}": f"shield_vec_{i}_{atom_idx}" for i in "xyz"})
    return df[["id"] + [f"shield_vec_{i}_{atom_idx}" for i in "xyz" for atom_idx in [0, 1]]]
####################################################################################################
# Settings
...
shelding_vec_feat_train = unpickle("../processed/v003/shelding_vec_feat_train.pkl")
# train_ = pd.read_csv("../input/train.csv")
# shelding_vec_feat_train = r = add_shelding_vec_feat(train_)
# try:
#     to_pickle("../processed/v003/shelding_vec_feat_train.pkl", shelding_vec_feat_train)
# except Exception as e:
#     print(e)
# del train_
# gc.collect()
train = train.merge(shelding_vec_feat_train, on="id", how="left")
...
X_test: pd.DataFrame = test.copy()#test[use_cols_revised].copy()

for c in ['shield_vec_x_0', 'shield_vec_x_1', 'shield_vec_y_0',
            'shield_vec_y_1', 'shield_vec_z_0', 'shield_vec_z_1',]:
    X_test[c] = np.nan
