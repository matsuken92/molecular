...
def get_train_test_data(use_prev=False, prev_data_version=None, prev_trial_no=None):
    ...
    if not use_prev:
        ...
        if DEBUG:
            train_path = Path(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H23J_sampled.pkl")
            test_path = Path(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H23J_sampled.pkl")
        ...
        if not sample_loaded:
            ...
            print("train loading")
            train: pd.DataFrame = unpickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H23J.pkl", )
            assert "scalar_coupling_constant" in train.columns
            print("test loading")
            test: pd.DataFrame = unpickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H23J.pkl", )
        ...
        if DEBUG and not sample_loaded:
            ...
            Path(f"../processed/{prev_data_version}/{prev_data_version}_{prev_trial_no}").mkdir(parents=True,
                                                                                                exist_ok=True)
            to_pickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H23J_sampled.pkl", train)
            to_pickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H23J_sampled.pkl", test)
            print("saved.")
...
####################################################################################################
# Data Loading
train, test = get_train_test_data(use_prev=True, prev_data_version="v003", prev_trial_no="075")
...
for seed in seed_list:
    ...
    for t in X['type'].unique():
        ...
        X_t = X.loc[X['type'] == t].copy()
        X_test_t = X_test.loc[X_test['type'] == t].copy()
        y_t = X_short.loc[X_short['type'] == t, 'target']
        y_fc_t = X_short.loc[X_short['type'] == t, 'fc']
        mol_name_t = mol_name.loc[X['type'] == t][X_t.index] if GROUP_K_FOLD else None
        print(f"X_t.shape: {X_t.shape}, X_test_t.shape: {X_test_t.shape}, y_t.shape: {y_t.shape}, y_fc_t: {y_fc_t.shape}")
