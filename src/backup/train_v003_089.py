...
def get_train_test_data(use_prev=False, prev_data_version=None, prev_trial_no=None):
    ...
    if not use_prev:
        ...
        if not sample_loaded:
            ...
            print("train loading")
            train: pd.DataFrame = unpickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_acsf.pkl", )
            assert "scalar_coupling_constant" in train.columns
            print("test loading")
            test: pd.DataFrame = unpickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_acsf.pkl", )
...
####################################################################################################
# Data Loading
train, test = get_train_test_data(use_prev=True, prev_data_version="v003", prev_trial_no="089")
...
# print("start loading mol_vec_df")
# mol_vec_df = unpickle("../processed/v003/mol_vec_df.pkl", )
# train = train.merge(mol_vec_df, on="molecule_name", how="left")
# test = test.merge(mol_vec_df, on="molecule_name", how="left")
# print("finished loading mol_vec_df")
#
...
#
# try:
#     folder = f"../processed/{DATA_VERSION}/{DATA_VERSION}_{TRIAL_NO}"
#     Path(folder).mkdir(parents=True, exist_ok=True)
#     to_pickle(f"{folder}/train_concat_{DATA_VERSION}_{TRIAL_NO}_acsf_X.pkl", X)
#     to_pickle(f"{folder}/train_concat_{DATA_VERSION}_{TRIAL_NO}_acsf_y.pkl", y)
#     to_pickle(f"{folder}/train_concat_{DATA_VERSION}_{TRIAL_NO}_acsf_y_fc.pkl", y_fc)
#     to_pickle(f"{folder}/train_concat_{DATA_VERSION}_{TRIAL_NO}_acsf_X_test.pkl", X_test)
# except:
#     print("save error.")
