...
mol_joint_type = ['1JHC', '1JHN', '2JHC', '2JHH', '2JHN', '3JHC', '3JHH', '3JHN']
...
mol_name = np.array(train_.molecule_name.values.tolist() + train_.molecule_name.values.tolist())
...
print(f"X_t.shape: {X.shape}, X_test_t.shape: {X_test.shape}")
if False:
    result_dict_lgb1 = train_model_regression(X=X,
                                              ...
                                              n_estimators=8000, fold_group=mol_name)
...
for t in X['type'].unique():
    print(f'Training of type {t}, {mol_joint_type[t]}')
    X_t = X.loc[X['type'] == t]
