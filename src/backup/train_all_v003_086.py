...
def get_train_test_data(use_prev=False, prev_data_version=None, prev_trial_no=None):
    ...
    if not use_prev:
        ...
        if not sample_loaded:
            ...
            print("train loading")
            train: pd.DataFrame = unpickle(f"{prev_folder}/train_compact_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J.pkl", )
            assert "scalar_coupling_constant" in train.columns
            print("test loading")
            test: pd.DataFrame = unpickle(f"{prev_folder}/test_compact_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J.pkl", )
        ...
        if DEBUG and not sample_loaded:
            ...
            Path(f"../processed/{prev_data_version}/{prev_data_version}_{prev_trial_no}").mkdir(parents=True,
                                                                                                exist_ok=True)
            to_pickle(f"{prev_folder}/train_compact_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J_sampled.pkl", train)
            to_pickle(f"{prev_folder}/test_compact_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J_sampled.pkl", test)
            print("saved.")
...
#seed_base = [2019]
#seed_list = np.array(seed_base) + 160
#seed_list = np.array(seed_base) + 161
#seed_list = np.array(seed_base) + 162
#seed_list = np.array(seed_base) + 163
#seed_list = np.array(seed_base) + 164
seed_list = np.array(seed_base) + 165

current_seed = seed_list[0] #-1
...
for seed in seed_list:
    ...
    if not USE_PREV_1st_MODEL:
        ...
        X['oof_fc'] = unpickle(f"../submit/{DATA_VERSION}_{TRIAL_NO}/train_oof_fc_{DATA_VERSION}_{TRIAL_NO}_{current_seed}.pkl")
        X_test['oof_fc'] = unpickle(f"../submit/{DATA_VERSION}_{TRIAL_NO}/test_oof_fc_{DATA_VERSION}_{TRIAL_NO}_{current_seed}.pkl")
    ...
    if seed==current_seed:
        processed_no = 2
        print(f"loading tmp_sub_{processed_no}")
        X_short_test = pd.read_csv(submit_path / f"tmp_sub_{processed_no}.csv")
        if not TRAIN_ALL_DATA:
            print(f"loading tmp_oof_{processed_no}")
            X_short = pd.read_csv(submit_path / f"tmp_oof_{processed_no}.csv")
    else:
