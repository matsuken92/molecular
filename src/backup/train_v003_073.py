...
def get_train_test_data(use_prev=False, prev_data_version=None, prev_trial_no=None):
    ...
    if not use_prev:
        ...
        if DEBUG:
            # v003_033
            train_path = Path(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_sampled.pkl")
            test_path = Path(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_sampled.pkl")
        ...
        if not sample_loaded:
            ...
            print("train loading")
            train: pd.DataFrame = unpickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ.pkl", )
            assert "scalar_coupling_constant" in train.columns
            print("test loading")
            test: pd.DataFrame = unpickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ.pkl", )
        ...
        if DEBUG and not sample_loaded:
            ...
            Path(f"../processed/{prev_data_version}/{prev_data_version}_{prev_trial_no}").mkdir(parents=True,
                                                                                                exist_ok=True)
            to_pickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_sampled.pkl", train)
            to_pickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_sampled.pkl", test)
            print("saved.")
...
#################################################################################################
# add circle feat
circle_feat_df = pd.read_csv("../processed/v003/circle_feat_df.csv", index_col=0).drop("circle_size", axis=1)
train = train.merge(circle_feat_df, on="molecule_name", how="left")
test = test.merge(circle_feat_df, on="molecule_name", how="left")

#################################################################################################
# add yukawa feat
yukawa_cols = ['dist_C_0', 'dist_C_1',
               'dist_C_2', 'dist_C_3', 'dist_C_4', 'dist_F_0', 'dist_F_1', 'dist_F_2',
               'dist_F_3', 'dist_F_4', 'dist_H_0', 'dist_H_1', 'dist_H_2', 'dist_H_3',
               'dist_H_4', 'dist_N_0', 'dist_N_1', 'dist_N_2', 'dist_N_3', 'dist_N_4',
               'dist_O_0', 'dist_O_1', 'dist_O_2', 'dist_O_3', 'dist_O_4']

def map_atom_yukawa(df, atom_idx, str_yukawa_df, yukawa_cols):
    yukawa_cols_dict = {c: f"yukawa_{c}_{atom_idx}" for c in yukawa_cols}

    output_cols = yukawa_cols_dict.values()
    yukawa_cols_dict.update({'atom': f'atom_{atom_idx}'})

    df = pd.merge(df, str_yukawa_df, how='left',
                  left_on=['molecule_name', f'atom_index_{atom_idx}'],
                  right_on=['molecule_name', 'atom_index'])

    df = df.drop('atom_index', axis=1)
    df = df.rename(columns=yukawa_cols_dict)
    return df, list(output_cols)

train_ = pd.read_csv("../input/train.csv")
test_ = pd.read_csv("../input/test.csv")
structures = pd.read_csv("../input/structures.csv")
struct_yukawa_df = pd.read_csv("../processed/v003/structures_yukawa.csv")
struct_yukawa_df = pd.concat([structures[["molecule_name", "atom_index", "atom"]], struct_yukawa_df], axis=1)
train_0, yukawa_cols_out_0 = map_atom_yukawa(train_, 0, struct_yukawa_df, yukawa_cols)
train_1, yukawa_cols_out_1 = map_atom_yukawa(train_, 1, struct_yukawa_df, yukawa_cols)
train = pd.concat([train, train_0[yukawa_cols_out_0], train_1[yukawa_cols_out_1]], axis=1)

test_0, yukawa_cols_out_0 = map_atom_yukawa(test_, 0, struct_yukawa_df, yukawa_cols)
test_1, yukawa_cols_out_1 = map_atom_yukawa(test_, 1, struct_yukawa_df, yukawa_cols)
test = pd.concat([test, test_0[yukawa_cols_out_0], test_1[yukawa_cols_out_1]], axis=1)
