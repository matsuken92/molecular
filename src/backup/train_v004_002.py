...
def train_model_regression(X, X_test, y, params, folds, model_type='lgb', eval_metric='mae', columns=None,
    ...
    print(f"X.shape: {X.shape}, fold_group: {len(fold_group)}")
...
def get_train_test_data(use_prev=False, prev_data_version=None, prev_trial_no=None, arg_target="z"):
    if use_prev:
    ...
    if not use_prev:
        ...
        if DEBUG:
            # v003_033
            train_path = Path(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_{arg_target}_basic_{arg_target}_sampled.pkl")
            test_path = Path(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_basic_sampled.pkl")
        ...
        if not sample_loaded:
            ...
            print("train loading")
            train: pd.DataFrame = unpickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_basic_{arg_target}_augmented.pkl", )
            assert "scalar_coupling_constant" in train.columns
            print("test loading")
            test: pd.DataFrame = unpickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_basic.pkl", )
        ...
        if DEBUG and not sample_loaded:
            ...
            Path(f"../processed/{prev_data_version}/{prev_data_version}_{prev_trial_no}").mkdir(parents=True,
                                                                                                exist_ok=True)
            to_pickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_{arg_target}_augmented_sampled.pkl", train)
            to_pickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_basic_sampled.pkl", test)
            print("saved.")
...
####################################################################################################
# Data Loading
train, test = get_train_test_data(use_prev=True, prev_data_version="v004", prev_trial_no="999", arg_target="z")
...
print(f"mol_name.shape; {mol_name.shape}")
...
for seed in seed_list:
    ...
    for t in X['type'].unique():
        ...
        print(f"mol_name_t: {len(mol_name_t)}")
