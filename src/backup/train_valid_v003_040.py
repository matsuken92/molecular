...
def train_lgb(X, y, X_test, seed, params, eval_metric, model_type="lgb"):
    # global t, X_t, result_dict_lgb, oof_log_mae
    print("start fitting")
...
if MODEL_TYPE == "lgb":
    ...
    for leaves in leaves_list:
        log_path = Path(f"../log/{DATA_VERSION}_{TRIAL_NO}_{leaves}")
        log_path.mkdir(parents=True, exist_ok=True)

        params["num_leaves"] = leaves
        score_list = train_lgb(X, y, X_test, seed, params, eval_metric, model_type=MODEL_TYPE)
    ...
    score_list = train_lgb(X_rgs, y, X_test_rgs, seed, params, eval_metric, model_type=MODEL_TYPE)
    print(f"mean score: {np.mean(score_list)}")
