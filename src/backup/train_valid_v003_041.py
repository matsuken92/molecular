...
def train_lgb_each_type(X, y, X_test, seed, params, eval_metric, model_type="lgb"):
    ...
    for t in X['type'].unique():
        ...
        if model_type == "lgb":
            ...
            result_dict["n_estimator"] = int(8000 * bairitsu)
...
def train_lgb(X, y, X_test, seed, params, eval_metric):
    print("start fitting")
    ...
    to_pickle(model_path / f"hold_out_model_{DATA_VERSION}_{TRIAL_NO}_{t}_{seed}.pkl", result_dict["model"])
    result_dict_path = submit_path / f'holdout_pred_{DATA_VERSION}_{TRIAL_NO}_{t}_{seed}.csv'
    ...
    if MODEL_TYPE == "mlp":
        ...
        sub.to_csv(submit_path/f'submission_t_{DATA_VERSION}_{TRIAL_NO}_{MODEL_TYPE}_{seed}.csv', index=False)
        print(sub.head())
        ...
        df_oof.to_csv(submit_path / f'oof_{DATA_VERSION}_{TRIAL_NO}_{MODEL_TYPE}_{seed}.csv', index=True)
        if LINE_MSG:
            send_message(f"{MODEL_TYPE}: finish train_{DATA_VERSION}_{TRIAL_NO}_{seed}, oof_log_mae: {oof_log_mae}")

    oof_log_mae = group_mean_log_mae(X_short['target'], X_short['oof'], X_short['type'], floor=1e-9)
    print(f"total oof_log_mae: {oof_log_mae}")
    print(f"finished. : {current_time()}")
    try:
        cv_score_msg = f"{DATA_VERSION}_{TRIAL_NO} finished {current_time()}, oof_log_mae: {oof_log_mae}"
        if LINE_MSG:
            send_message(cv_score_msg)
...
result_dict_list = []
eval_metric = "mae" if DEBUG else "group_mae"

if MODEL_TYPE == "lgb":
    #leaves_list = [2, 4, 8, 16] if DEBUG else [64, 96, 128, 192, 256]
    #leaves_list = [2, 4, 8, 16] if DEBUG else [4, 6, 8, 12, 16, 32, 48, 64, 96, 128]
    leaves_list = [2, 4, 8, 16] if DEBUG else [12]
    for leaves in leaves_list:
        log_path = Path(f"../log/{DATA_VERSION}_{TRIAL_NO}/num_leaves_{leaves}")
        log_path.mkdir(parents=True, exist_ok=True)

        params["num_leaves"] = leaves
        score_list = train_lgb_each_type(X, y, X_test, seed, params, eval_metric, model_type=MODEL_TYPE)
        print(f"mean score: {np.mean(score_list)}")
        result_dict_list += [{"num_leaves": leaves, "score_list": score_list, "score_mean": np.mean(score_list)}]

    result_df = pd.DataFrame(result_dict_list)
    result_df.to_csv(log_path / f"result_df_{MODEL_TYPE}.csv")

if MODEL_TYPE == "lgb_holdout":
    params["num_leaves"] = 8
    train_lgb(X, y, X_test, seed, params, eval_metric)
