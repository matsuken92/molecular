...
if False:
    ...
    train = unpickle(save_path/f"train_concat_v003_{DATA_VERSION}_{TRIAL_NO}.pkl", )
    test = unpickle(save_path/f"test_concat_v003_{DATA_VERSION}_{TRIAL_NO}.pkl", )
...
for t in X['type'].unique():
    ...
    print(f"X_t.shape: {X_t.shape}, X_test_t.shape: {X_test_t.shape}, len(y_t): {len(y_t)}")
    for oof_type in ['fc', 'sd', 'dso', 'pso']:

        y_t_s = y_dict[oof_type].loc[X_t.index]
        print(f"oof_type: {oof_type},  X_t.shape: {X.shape}, X_test_t.shape: {X_test.shape}, y_t_s: {len(y_t_s)}")
        ...
        if True:
            result_dict_lgb1 = train_model_regression(X=X_t,
                                                      ...
                                                      n_estimators=8000)
            X_t[f'oof_{oof_type}_{t}'] = result_dict_lgb1['oof']
            X_test_t[f'oof_{oof_type}_{t}'] = result_dict_lgb1['prediction']
            to_pickle(submit_path / f"train_oof_{oof_type}_{DATA_VERSION}_{TRIAL_NO}_{t}.pkl", X_t[f'oof_{oof_type}_{t}'])
            to_pickle(submit_path / f"test_oof_{oof_type}_{DATA_VERSION}_{TRIAL_NO}_{t}.pkl", X_test_t[f'oof_{oof_type}_{t}'])
            to_pickle(model_path / f"first_model_{oof_type}_list_{DATA_VERSION}_{TRIAL_NO}_{t}.pkl",
                      result_dict_lgb1["models"])
        else:
            X_t[f'oof_{oof_type}_{t}'] = unpickle(submit_path / f"train_oof_{oof_type}_{DATA_VERSION}_{TRIAL_NO}_{t}.pkl", )
            X_test_t[f'oof_{oof_type}_{t}'] = unpickle(submit_path / f"test_oof_{oof_type}_{DATA_VERSION}_{TRIAL_NO}_{t}.pkl", )
    ...
    X_short.to_csv(submit_path/f"tmp_oof_{DATA_VERSION}_{TRIAL_NO}_{t}.csv")
    X_short_test.to_csv(submit_path/f"tmp_sub_{DATA_VERSION}_{TRIAL_NO}_{t}.csv")
