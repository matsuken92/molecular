...
def train_model_regression(X, X_test, y, params, folds, model_type='lgb', eval_metric='mae', columns=None,
    ...
    for fold_n, (train_index, valid_index) in enumerate(folds.split(X, groups=fold_group)):
        ...
        if model_type == 'lgb' and plot_feature_importance:
            ...
            try:
                fold_importance.to_csv(f"./tmp_importance_{ fold_n + 1}.csv")
            except:
                print("failed save importance.")
...
def get_train_test_data(use_prev=False, prev_data_version=None, prev_trial_no=None):
    ...
    if not use_prev:
        ...
        if DEBUG:
            train_path = Path(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J_sampled.pkl")
            test_path = Path(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J_sampled.pkl")
        ...
        if not sample_loaded:
            ...
            print("train loading")
            train: pd.DataFrame = unpickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J.pkl", )
            assert "scalar_coupling_constant" in train.columns
            print("test loading")
            test: pd.DataFrame = unpickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J.pkl", )
        ...
        if DEBUG and not sample_loaded:
            ...
            Path(f"../processed/{prev_data_version}/{prev_data_version}_{prev_trial_no}").mkdir(parents=True,
                                                                                                exist_ok=True)
            to_pickle(f"{prev_folder}/train_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J_sampled.pkl", train)
            to_pickle(f"{prev_folder}/test_concat_{prev_data_version}_{prev_trial_no}_yiemon_123J_HnJ_H123J_sampled.pkl", test)
            print("saved.")
...
train_id = pd.read_csv("../input/train.csv").id
...
print(f"train.shape: {train.shape}")
print("node_train 1")
train = map_node(train, node_df, 1)
print(f"train.shape: {train.shape}")
print("node_test 0")
test = map_node(test, node_df, 0)
print(f"test.shape: {test.shape}")
print("node_test 1")
test = map_node(test, node_df, 1)
print(f"test.shape: {test.shape}")
print("node finished")
del node_df
gc.collect()

print("loading train_dist_all_you_need_feat")
train_dist_all_you_need_feat = unpickle("../processed/v003/train_dist_all_you_need_feat.pkl", )
train = pd.concat([train, train_dist_all_you_need_feat], axis=1)
print(f"train.shape: {train.shape}")
del train_dist_all_you_need_feat

print("loading test_dist_all_you_need_feat")
test_dist_all_you_need_feat = unpickle("../processed/v003/test_dist_all_you_need_feat.pkl", )
test = pd.concat([test, test_dist_all_you_need_feat], axis=1)
print(f"test.shape: {test.shape}")
del test_dist_all_you_need_feat
gc.collect()

print("loading train_kernel_plus_more_feats")
train_kernel_plus_more_feats = unpickle("../processed/v003/train_kernel_plus_more_feats.pkl", )
dup_cols = [c for c in train_kernel_plus_more_feats.columns if c not in use_cols.duplicate_cols]
train_kernel_plus_more_feats = train_kernel_plus_more_feats[dup_cols]
train = pd.concat([train, train_kernel_plus_more_feats], axis=1)
print(f"train.shape: {train.shape}")
del train_kernel_plus_more_feats

print("loading train_kernel_plus_more_feats")
tset_kernel_plus_more_feats = unpickle("../processed/v003/test_kernel_plus_more_feats.pkl", )
tset_kernel_plus_more_feats = tset_kernel_plus_more_feats[dup_cols]
test = pd.concat([test, tset_kernel_plus_more_feats], axis=1)
print(f"test.shape: {test.shape}")
del tset_kernel_plus_more_feats
gc.collect()

assert len(train) == 4658147
assert len(test)  == 2505542

use_cols_revised = [c for c in use_cols.good_columns if c not in use_cols.remove_cols + use_cols.qm9_cols]
...
for seed in seed_list:
    ...
    if not USE_PREV_1st_MODEL:
        ...
        X['oof_fc'] = unpickle("../submit/v003_067/train_oof_fc_v003_067_2069.pkl")
        X_test['oof_fc'] = unpickle("../submit/v003_067/test_oof_fc_v003_067_2069.pkl")
    ...
    if seed==current_seed:
        ...
        X_short = pd.read_csv(submit_path / f"tmp_oof_4_2069.csv")
        X_short_test = pd.read_csv(submit_path / f"tmp_sub_4_2069.csv")
    else:
