...
train = unpickle("../processed/v003/v003_104/train_compact_v003_104_compact.pkl")
...
# del feat_test
#
# feat_train = unpickle("../processed/v003/nearest3Js_train.pkl")
# feat_test = unpickle("../processed/v003/nearest3Js_test.pkl")
# train = pd.concat([train, feat_train], axis=1)
# test  = pd.concat([test, feat_test], axis=1)
# assert len(train) == 4658147
# assert len(test)  == 2505542
...
# Path("../processed/v003/v003_104").mkdir(parents=True, exist_ok=True)
# to_pickle("../processed/v003/v003_104/train_compact_v003_104_compact.pkl", train[use_cols_revised])
# to_pickle("../processed/v003/v003_104/test_compact_v003_104compact.pkl", test[use_cols_revised])
#
# print("Data Saved!!!")
# assert False
...
for seed in seed_list:
    ...
    if TRAIN_ALL_DATA or CV_FOLD:
        ...
        submit_file_path = str(submit_path / f'submission_t_{DATA_VERSION}_{TRIAL_NO}_{seed}.csv')
        sub.to_csv(submit_file_path, index=False)

        cmd = f"scp -i ~/key/google_compute_engine -r {submit_file_path} " \
              f"10.138.0.18:/home/kenichi.matsui/work/molecular/submit/{DATA_VERSION}_{TRIAL_NO}/"
        try:
            os.system(cmd)
        except Exception as e:
            print("error in transfer submit file")
            print(e)
