...
def strategy_func(current_lr, eval_history, best_round):
    """次のラウンドで用いる学習率を決定するための関数 (これを好きに改造する)
    ...
    current_lr = current_lr or 0.5

    # 300 ラウンド毎に学習率を0.7倍
    if len(eval_history) % 500 == 0:
        current_lr *= 0.92
...
def train_model_regression(X, X_test, y, params, folds, model_type='lgb', eval_metric='mae', columns=None,
    ...
    for fold_n, (train_index, valid_index) in enumerate(folds.split(X, groups=fold_group)):
        ...
        if model_type == 'lgb':
            ...
            print("DynamicallyLrControllerCallback applied.")
            model.fit(X_train, y_train, callbacks=[dylr_cb],
                      eval_set=[(X_train, y_train), (X_valid, y_valid)],
                      eval_metric=metrics_dict[eval_metric]['lgb_metric_name'],
                      verbose=verbose, early_stopping_rounds=early_stopping_rounds)
